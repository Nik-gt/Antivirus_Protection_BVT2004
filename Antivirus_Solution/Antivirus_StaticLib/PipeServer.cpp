...
public:
    ...
    PipeServer() {
        ...
        hPipe = CreateNamedPipe(TEXT("\\\\.\\pipe\\anti"),
            ...
            NULL);
        //Create Named Pipe for write to client
        hPipe0 = CreateNamedPipe(TEXT("\\\\.\\pipe\\anti0"),
            PIPE_ACCESS_DUPLEX,
            PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT,
            PIPE_UNLIMITED_INSTANCES,
            1024 * 16,
    ...
    std::string PipeRead()
    {
        ...
        if (!success || dwRead == 0) 
        { 
            std::cout << "Pipe closed/error." << std::endl; WaitClientConnection(); 
    ...
    void PipeWrite(std::string message) //Не строки int float duble char uint 8 uint 16
    {
        ...
        {
            WriteFile(hPipe0, buffer, strlen(buffer), &dwWritten, NULL);
        }
    }    
    /*
    void PipeWrite(std::string message) //Не строки int float duble char uint 8 uint 16
